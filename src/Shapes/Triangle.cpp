...
bool Triangle::Intersect(const Ray3f& ray, SurfaceInteraction* isec) const
{
	...
	if (FLAT_SHADING)
	{
		N = glm::normalize(glm::cross(_vertices[1]->Position - _vertices[0]->Position,
			_vertices[2]->Position - _vertices[1]->Position));
	}
	else
	{
		N = glm::normalize(bary_interp(bary_coord, _vertices[0]->Normal, 
			_vertices[1]->Normal, _vertices[2]->Normal));
	}
...
SurfaceInfo Triangle::SampleSurface(const Vector2f& sample) const
{
	...
	Point3f pos = baryCoord.x * _vertices[0]->Position
		+ baryCoord.y * _vertices[1]->Position 
		+ baryCoord.z * _vertices[2]->Position;
	Normal3f N;
	if (FLAT_SHADING)
	{
		N = glm::normalize(glm::cross(_vertices[1]->Position - _vertices[0]->Position, 
	...
	{
		N = glm::normalize(bary_interp(baryCoord, _vertices[0]->Normal, 
			_vertices[1]->Normal, _vertices[2]->Normal));
	}
